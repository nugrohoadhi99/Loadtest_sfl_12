...
export default function () {
  ...
  group("SFL FE FLOW (AUTH)", () => {
    ...
    const r6 = http.get(
      `${FE_BASE}/sfl/quiz/12/finish`,
      paramsWithName("fe_quiz-finish", user.idx, authHeader) // ✅ authHeader wajib
    );
    ...
    check(r6, { "r6 OK+AUTH": (r) => ok(r) && mustBeLoggedInPage(r) });
    ...
    if (__VU <= MAX_LOG_VU) {
      console.log(
        `VU ${__VU} r1=${statusOk(r1)} r2=${statusOkAuth(r2)} r3=${statusOkAuth(
          r3
        )} r4=${statusOkAuth(r4)} r5=${statusOkAuth(r5)} r6=${statusOkAuth(r6)}`
      );
    }
  });
