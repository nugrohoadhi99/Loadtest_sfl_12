...
// PER-VU STATE
// =======================
let authHeader;            // token cache per VU
let didLoginAttempt = false; // ✅ biar gak login terus-terusan
...
export default function () {
  ...
  // ✅ LOGIN hanya SEKALI per VU
  if (!authHeader && !didLoginAttempt) {
    didLoginAttempt = true;
    authHeader = loginAndGetToken(user);

    if (!authHeader) {
      if (__VU <= MAX_LOG_VU) console.log(`VU ${__VU} login=FAIL user=${user.email}`);
      return; // stop, jangan spam login di iterasi berikutnya
    }

    if (__VU <= MAX_LOG_VU) console.log(`VU ${__VU} login=OK user=${user.email}`);
  }

  // kalau login gagal tadi, authHeader tetap null, stop terus (no relogin)
